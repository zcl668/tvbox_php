...
delete_project() {
    ...
    echo -e "${YELLOW}1. 项目目录: $REPO_DIR/$PROJECT_NAME${NC}"
    ...
    find "$REPO_DIR" -name "*.backup_*" -type f -delete 2>/dev/null
    find "." -name "*.backup_*" -type f -delete 2>/dev/null
    echo -e "${GREEN}备份文件清理完成。${NC}"
    
    # 检查是否在项目目录中，如果是则退出到上级目录
    if [[ $(pwd) == *"$PROJECT_NAME"* ]]; then
        cd ..
        echo -e "${YELLOW}已退出项目目录。${NC}"
    fi
