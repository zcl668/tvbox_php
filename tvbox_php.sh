...
if ! pgrep -f 'php -S 0.0.0.0:8901' > /dev/null; then
    cd '$PHP_ROOT' && nohup php -S 0.0.0.0:8901 > /dev/null 2>&1 &
    echo \"PHP服务启动中...\"
fi"
if ! grep -q "php -S 0.0.0.0:8901" "$TARGET_FILE"; then
...
case "\\$1" in
    start)
        cd "$PHP_ROOT" && nohup php -S 0.0.0.0:8901 > /dev/null 2>&1 &
        echo "PHP服务已启动: http://$SERVER_IP:8901"
        ;;
    stop)
        pkill -f "php -S 0.0.0.0:8901"
        echo "PHP服务已停止"
        ;;
    status)
        if pgrep -f "php -S 0.0.0.0:8901" > /dev/null; then
            echo "PHP服务运行中: http://$SERVER_IP:8901"
        else
            echo "PHP服务未运行"
        fi
        ;;
    restart)
        pkill -f "php -S 0.0.0.0:8901"
        cd "$PHP_ROOT" && nohup php -S 0.0.0.0:8901 > /dev/null 2>&1 &
        echo "PHP服务已重启: http://$SERVER_IP:8901"
        ;;
    *)
        echo "用法: tvbox-php {start|stop|status|restart}"
        echo "根目录: $PHP_ROOT"
        echo "访问地址: http://$SERVER_IP:8901"
        ;;
esac
EOF
